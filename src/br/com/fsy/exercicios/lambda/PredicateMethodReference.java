...
public class PredicateMethodReference {
	...
	public static void main(String... args) {
		...
		List<Box> greenApples = filter(inventory, PredicateMethodReference::isGreenApple);
		System.out.println(greenApples);

		List<Box> heavyApples = filter(inventory, PredicateMethodReference::isHeavyApple);
		...
		List<Box> weirdApples = filter(inventory, (Box b) -> b.getWeight() < 80 || "brown".equals(b.getColor()));
		System.out.println(weirdApples);
		
		Thread t = new Thread(() -> System.out.println("Started!"));
		t.start();
	}
